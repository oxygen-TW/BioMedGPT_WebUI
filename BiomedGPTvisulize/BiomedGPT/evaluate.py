...
def main(cfg: DictConfig, **kwargs):
    ...
    for model, ckpt_path in zip(models, utils.split_paths(cfg.common_eval.path)):
        ...
        # === Attention hook ===
        GLOBAL_ATTENTION_ALL = {}  # 存每一層的 hook 結果

        def make_hook_fn(layer_idx):
            def hook_fn(module, input, output):
                if isinstance(output, tuple) and len(output) > 1 and output[1] is not None:
                    GLOBAL_ATTENTION_ALL[layer_idx] = output[1].detach().cpu()
                #     print(f"✅ Hooked attention at layer {layer_idx} — shape: {output[1].shape}")
                # else:
                #     print(f"❌ Layer {layer_idx} has no usable attention output.")
            return hook_fn

        # Register Hook
        num_layers = len(models[0].encoder.layers)
        for i in range(num_layers):
            models[0].encoder.layers[i].self_attn.register_forward_hook(make_hook_fn(i))
            print(f"📌 Registered hook on encoder layer {i}")
    ...
    attn_tensor = GLOBAL_ATTENTION_ALL[layer]
    attn = attn_tensor[head, 0]
    attn_map = attn[0, 1:] if attn.shape[0] == attn.shape[1] else attn[1:]
    ...
    df = pd.read_csv("/home/harper112/biomedGPT/BiomedGPT/datasets/finetuning/PathVQA/iu_xray_test.tsv", sep='\t', header=None)
    img_base64 = df.iloc[idx, 4]
    img_id = df.iloc[idx, 1]
    img_bytes = base64.b64decode(img_base64)
    image = Image.open(BytesIO(img_bytes)).resize((224, 224))
    ...
    output_path = f"attention_vis/{img_id}_L{layer}_H{head}.png"
