...
def objects_are_equal(a, b):
    """
    Recursively check whether two objects are equal.
    Supports nested structures like dicts, lists, tuples.
    """
    if type(a) != type(b):
        return False
    if isinstance(a, dict):
        if a.keys() != b.keys():
            return False
        return all(objects_are_equal(a[k], b[k]) for k in a)
        ...
        return a == b
def spawn_and_init(fn, args=(), nprocs=torch.cuda.device_count()):
    """
    Launch `nprocs` processes using `torch.multiprocessing.spawn`, and initialize
    distributed training in each process. The `distributed_utils` module must
    implement `call_main` which wraps the actual entry point.
    """
    # used by distributed_utils.call_main
    distributed_utils.call_main = fn
    mp.spawn(distributed_utils.call_main, args=(args,), nprocs=nprocs)
